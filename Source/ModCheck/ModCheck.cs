...
#pragma warning disable CS0649
    ...
    public class isVersion : ModCheckBase
    {
        ...
        private string getHighestVersion(List<Version> versions)
        {
            Version currentHighest = new Version("0.0.0.0");
            foreach (Version currentVersion in versions)
            {
                if (currentVersion > currentHighest)
                {
                    currentHighest = currentVersion;
                }
            }
        ...
        protected override bool isTestPassed()
        {
            ...
            foreach (string loopName in altModNames)
            {
                ...
                if (index != -1)
                {
                    string currentStr = getHighestVersion(ModsConfig.ActiveModsInLoadOrder.ElementAt(index).SupportedVersionsReadOnly);
        ...
        protected override string getDefaultErrorString()
        {
            ...
            string readVersion = getHighestVersion(ModsConfig.ActiveModsInLoadOrder.ElementAt(index).SupportedVersionsReadOnly);
